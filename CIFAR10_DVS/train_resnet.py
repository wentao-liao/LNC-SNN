...
def parse_args():
    ...
    parser.add_argument('--dataset', default='CIFAR10_DVS', help='dataset')
...
def train_early_exit_one_epoch(model, criterion, optimizer, data_loader, device, epoch, print_freq,scaler=None,T_train=None, aug=None, trival_aug=None, mixup_fn=None,
                               exit_loss_weights={'exit1': 0.05, 'exit2': 0.05, 'exit3': 0.1, 'exit4': 0.4, 'main': 0.4},
                               ):
    ...
    for image, target in metric_logger.log_every(data_loader, print_freq, header):
        ...
        if mixup_fn is not None:
            for exit_name in ['exit1', 'exit2', 'exit3', 'exit4', 'main']:
                acc1, acc5 = utils.accuracy(outputs[exit_name], target_for_compu_acc, topk=(1, 5))
...
def main(args):
    ...
    if args.model in ms_resnet.__dict__:
        model = ms_resnet.__dict__[args.model](T=args.T, num_classes=args.num_classes)
    else:
